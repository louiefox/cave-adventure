-- DRAW FUNCTIONS --
function CAVEADVENTURE.FUNC.GetTheme( themeNum, alpha )
    local color = CAVEADVENTURE.CONFIG.Themes[themeNum] or Color( 255, 255, 255 )
    return alpha and Color( color.r, color.g, color.b, alpha ) or color
end

function CAVEADVENTURE.FUNC.ScreenScale( number )
...
function CAVEADVENTURE.FUNC.DrawPartialRoundedBox( cornerRadius, x, y, w, h, color, roundedBoxW, roundedBoxH, roundedBoxX, roundedBoxY )
	startStencil()
...
function CAVEADVENTURE.FUNC.DrawPartialRoundedBoxEx( cornerRadius, x, y, w, h, color, roundedBoxW, roundedBoxH, roundedBoxX, roundedBoxY, roundTopLeft, roundTopRight, roundBottomLeft, roundBottomRight )
	startStencil()
...
CAVEADVENTURE.MaskMaterial = CreateMaterial("!caveadventure_mask","UnlitGeneric",{
	["$translucent"] = 1,
	["$vertexalpha"] = 1,
	["$alpha"] = 1,
})
...
local function drawRoundedMask( cornerRadius, x, y, w, h, drawFunc, roundTopLeft, roundTopRight, roundBottomLeft, roundBottomRight )
	...
	surface.SetDrawColor( 255, 255, 255, 255 ) 
	surface.SetMaterial( CAVEADVENTURE.MaskMaterial ) 
	render.SetMaterial( CAVEADVENTURE.MaskMaterial )
...
-- CIRCLE/ARC STUFF --
function CAVEADVENTURE.FUNC.DrawCircle( x, y, radius, color )
...
function CAVEADVENTURE.FUNC.PrecachedArc( cx, cy, radius, thickness, startang, endang, roughness )
	local triarc = {}
...
function CAVEADVENTURE.FUNC.DrawCachedArc( arc, color )
	draw.NoTexture()
...
function CAVEADVENTURE.FUNC.DrawArc( cx, cy, radius, thickness, startang, endang, color )
	CAVEADVENTURE.FUNC.DrawCachedArc( CAVEADVENTURE.FUNC.PrecachedArc( cx, cy, radius, thickness, startang, endang ), color )
end

local radiusAnim, fadeAnim, endRadius = 0.2, 0.2, 0
function CAVEADVENTURE.FUNC.DrawClickCircle( panel, w, h, color, cornerRadius, overrideX, overrideY )
	...
	if( panel.doClickAnim ) then
		...
		if( cornerRadius ) then
			CAVEADVENTURE.FUNC.DrawRoundedMask( cornerRadius, 0, 0, w, h, function()
				CAVEADVENTURE.FUNC.DrawCircle( x, y, radius, radius )
			end )
		else
			CAVEADVENTURE.FUNC.DrawCircle( x, y, radius, radius )
		end
...
function CAVEADVENTURE.FUNC.DrawGradientBox(x, y, w, h, direction, ...)
	local colors = {...}
...
function CAVEADVENTURE.FUNC.DrawBlur( p, a, d )
...
local function GetImageFromURL( url, failFunc )
    ...
    if( not file.Exists( "caveadventure/images", "DATA" ) ) then
        file.CreateDir( "caveadventure/images" )
    end
    
    if( file.Exists( "caveadventure/images/" .. CRC .. "." .. Extension, "DATA" ) ) then
        CAVEADVENTURE.TEMP.CachedMaterials[url] = Material( "data/caveadventure/images/" .. CRC .. "." .. Extension )

        if( failFunc ) then
            failFunc( CAVEADVENTURE.TEMP.CachedMaterials[url], key )
        end

        return CAVEADVENTURE.TEMP.CachedMaterials[url], key
    else
        http.Fetch( url, function( body )
            file.Write( "caveadventure/images/" .. CRC .. "." .. Extension, body )
            CAVEADVENTURE.TEMP.CachedMaterials[url] = Material( "data/caveadventure/images/" .. CRC .. "." .. Extension )
...
CAVEADVENTURE.TEMP.CachedMaterials = {}

function CAVEADVENTURE.FUNC.CacheImageFromURL( url, failFunc )
    CAVEADVENTURE.TEMP.CachedMaterials[url] = false

    if( not CAVEADVENTURE.TEMP.CachedMaterials[url] ) then
        CAVEADVENTURE.TEMP.CachedMaterials[url] = GetImageFromURL( url, failFunc )
    end
end

CAVEADVENTURE.TEMP.GetImagesQueue = CAVEADVENTURE.TEMP.GetImagesQueue or {}
local function RunGetImagesQueue()
	local queueItem = CAVEADVENTURE.TEMP.GetImagesQueue[1]
	if( queueItem ) then
		table.remove( CAVEADVENTURE.TEMP.GetImagesQueue, 1 )

		local url, onGetFunc = queueItem[1], queueItem[2]
		if( CAVEADVENTURE.TEMP.CachedMaterials[url] ) then
			onGetFunc( CAVEADVENTURE.TEMP.CachedMaterials[url] )
			RunGetImagesQueue()
		else
			CAVEADVENTURE.FUNC.CacheImageFromURL( url, onGetFunc )

			timer.Create( "CAVEADVENTURE.Timer.GetImageLoad", 0.2, 1, function()
				RunGetImagesQueue()
			end )
		end
	end
...
function CAVEADVENTURE.FUNC.GetImage( url, onGetFunc )
	table.insert( CAVEADVENTURE.TEMP.GetImagesQueue, { url, onGetFunc } )
	
	if( timer.Exists( "CAVEADVENTURE.Timer.GetImageLoad" ) ) then return end
	RunGetImagesQueue()
end
...
function CAVEADVENTURE.FUNC.TextWrap(text, font, maxWidth)
    local totalWidth = 0
...
function CAVEADVENTURE.FUNC.DrawNonParsedText(text, font, x, y, color, xAlign)
